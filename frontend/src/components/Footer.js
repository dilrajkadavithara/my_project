...
// SVG ICONS (unchanged, use your previous ones or these as-is)
const PhoneIcon = () => (
  <svg width="28" height="28" fill="currentColor" aria-hidden="true"><path d="M2.25 3.5a2 2 0 0 1 2-2h2.09a2 2 0 0 1 1.93 1.39l.56 1.68a2 2 0 0 1-.45 2.11l-.98.98a15.72 15.72 0 0 0 6.36 6.36l.98-.98a2 2 0 0 1 2.11-.45l1.68.56a2 2 0 0 1 1.39 1.93v2.09a2 2 0 0 1-2 2A17.5 17.5 0 0 1 2.25 3.5z"/></svg>
);
const EmailIcon = () => (
  <svg width="28" height="28" fill="currentColor" aria-hidden="true"><path d="M2.5 4A2.5 2.5 0 0 1 5 1.5h10A2.5 2.5 0 0 1 17.5 4v12A2.5 2.5 0 0 1 15 18.5H5A2.5 2.5 0 0 1 2.5 16V4zm1.75 1L10 9.5 15.75 5H4.25zm-.25 1.618V16A1 1 0 0 0 5 17h10a1 1 0 0 0 1-1V6.618l-6 4.364-6-4.364z"/></svg>
);
const ClockIcon = () => (
  <svg width="28" height="28" fill="currentColor" aria-hidden="true"><path d="M10 2a8 8 0 1 1 0 16A8 8 0 0 1 10 2zm0 1.5A6.5 6.5 0 1 0 10 16.5 6.5 6.5 0 0 0 10 3.5zm.75 3.75a.75.75 0 0 0-1.5 0v3.38l-2.14 2.13a.75.75 0 0 0 1.06 1.06l2.37-2.36A.75.75 0 0 0 10.75 9V7.25z"/></svg>
);
const LocationIcon = () => (
  <svg width="28" height="28" fill="currentColor" aria-hidden="true"><path d="M10 2a6 6 0 0 1 6 6c0 4.63-5.23 9.15-5.45 9.34a.75.75 0 0 1-1.1 0C9.23 17.15 4 12.63 4 8A6 6 0 0 1 10 2zm0 8.25a2.25 2.25 0 1 0 0-4.5 2.25 2.25 0 0 0 0 4.5z"/></svg>
);
const FacebookIcon = () => (
  <svg width="32" height="32" fill="currentColor" aria-hidden="true" viewBox="0 0 320 512"><path d="M279.14 288l14.22-92.66h-88.91v-60.13c0-25.35 12.42-50.06 52.24-50.06H293V6.26S259.5 0 225.36 0c-73.22 0-121.08 44.38-121.08 124.72V195.3H22.89V288h81.39v224h100.2V288z"/></svg>
);
const InstagramIcon = () => (
...
const Footer = ({ websiteContent }) => {
  // Social links
  const socials = [
    { icon: <FacebookIcon />, url: websiteContent['social_facebook_url']?.value, label: "Facebook" },
    { icon: <InstagramIcon />, url: websiteContent['social_instagram_url']?.value, label: "Instagram" },
    { icon: <WhatsappIcon />, url: websiteContent['social_whatsapp_phone']?.value ? `https://wa.me/${websiteContent['social_whatsapp_phone'].value.replace(/\D/g, '')}` : null, label: "WhatsApp" },
  ];
          ...
          <div className="footer-card direct-contact">
            ...
            <div className="footer-contact-list">
              <div className="flex-align-center">
                ...
                <a href={websiteContent['footer_phone']?.value ? `tel:${websiteContent['footer_phone'].value}` : "#"}>{phone}</a>
              </div>
              <div className="flex-align-center">
                <span className="footer-icon"><EmailIcon /></span>
                <a href={websiteContent['footer_email']?.value ? `mailto:${websiteContent['footer_email'].value}` : "#"}>{email}</a>
              </div>
            </div>
          </div>
            ...
            <div className="social-icons">
              {anySocials
                ? socials.filter(s => s.url).map((s) => (
                    <a key={s.label} href={s.url} target="_blank" rel="noopener noreferrer" aria-label={s.label} className="social-icon">
                      {s.icon}
                    </a>
                  ))
